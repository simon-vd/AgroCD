...
3. [Kind Cluster Configuration](#1-kind-cluster-configuration-kindexposeyaml)
4. [Namespace Definition](#2-namespace-definition-namespaceyaml)
5. [Database Layer](#3-database-layer-databaseyaml)
6. [Backend Layer](#4-backend-layer-backendyaml)
7. [Frontend Layer](#5-frontend-layer-frontendyaml)
8. [Ingress & TLS](#6-ingress--tls-ingressyaml)
9. [GitOps with Argo CD](#7-gitops-with-argo-cd-argo-cdyaml)
10. [Prometheus Ingress Configuration](#8-prometheus-ingress-configuration-prometheusyaml)
11. [Application Dockerfiles and Assets](#9-application-dockerfiles-and-assets)
12. [Helm Installation Commands](#10-helm-installation-commands)
...
| **Prometheus**       | [https://monitor.milestone2.example.com:31740/query](https://milestone2.example.com:30900)  | Monitoring and metrics dashboard    |
...
## 8. Prometheus Ingress Configuration (`prometheus.yaml`)

To ensure monitoring data is accessed securely, we expose Prometheus via the Ingress controller using HTTPS, rather than relying on the insecure NodePort.
...
spec:
  ...
  tls:
    - hosts:
        - monitor.milestone2.example.com
      secretName: prometheus-tls
  rules:
    - host: monitor.milestone2.example.com
      http:
        paths:
          - path: /
            pathType: Prefix
...
### Line-by-Line Explanation:
- **`namespace: monitoring`**: Crucial—this Ingress must live in the same namespace as the Prometheus service.
- **`annotations`**:
  - **`cert-manager.io/cluster-issuer`**: Instructs Cert-Manager to generate a certificate for this specific subdomain using our existing cluster issuer.
- **`spec`**:
  - **`ingressClassName: traefik`**: Routes traffic through our Traefik gateway.
  - **`tls`**: Defines the HTTPS configuration.
    - **`hosts`**: The specific domain (`monitor.milestone2.example.com`) we are securing.
    - **`secretName: prometheus-tls`**: Where the generated SSL certificate will be stored.
  - **`rules`**:
    - **`host`**: Matches requests coming to `monitor.milestone2.example.com`.
    - **`path: /`**: Routes all traffic for this domain.
    - **`backend`**: Forward traffic to the Prometheus service.
    - **`name: prometheus-kube-prometheus-prometheus`**: The actual service name installed by the Helm chart.
    - **`port: 9090`**: The internal port Prometheus listens on.

---

## 9. Application Dockerfiles and Assets
...
- `"name": "backend"`: The name of the application.
- `"version": "1.0.0"`: The current version of the application.
- `"description": ""`: A brief description of the project.
- `"main": "app.js"`: The entry point of the application.
- `"scripts": { "start": "node app.js" }`: Defines executable commands; `npm start` runs the app.
- `"dependencies":`: Lists the external libraries required for the application.
- `"express": "^4.18.2"`: The web framework used to build the API.
- `"mysql": "^2.18.1"`: The driver used to connect to the MySQL database.
...
- `<meta name="viewport" ... />`: Ensures the page renders correctly on mobile devices.
- `<title>Milestone 2</title>`: Sets the title shown in the browser tab.
- `</head>`: Closes the head section.
- `<body>`: Contains the visible content of the page.
- `<h1>...</h1>`: The main heading of the page.
- `<span id="user">Loading...</span>`: A placeholder span that will be updated with the username fetched from the API.
- `<p>Container ID: ...</p>`: A paragraph displaying the container ID.
- `<strong id="cid">Loading...</strong>`: A placeholder strong element that will be updated with the container ID fetched from the API.
- `<script>`: Begins the JavaScript section.
- `fetch("/api/name")`: Initiates a network request to the `/api/name` endpoint.
- `.then(r => r.json())`: Converts the response to JSON format.
- `.then(d => ...)`: Updates the 'user' element with the name received from the API.
- `.catch(...)`: Handles any errors by displaying "error".
- `fetch("/api/container-id")`: Initiates a request to the `/api/container-id` endpoint.
- `.then(r => r.json())`: Converts the response to JSON.
- `.then(d => ...)`: Updates the 'cid' element with the container ID.
- `.catch(...)`: Handles errors.
- `</script>`: Ends the script section.
- `</body>`: Closes the body.
- `</html>`: Ends the HTML document.

### Frontend Web Server Config (`frontend/lighttpd.conf`)

This file configures the Lighttpd server to serve the static files and proxy API requests.
...
- `server.document-root = ...`: Sets the directory from which potentially served files are searched.
- `server.port = 80`: The port the web server listens on.
- `mimetype.assign = ...`: Maps file extensions to mime-types, ensuring browsers interpret files correctly.
- `server.error-handler-404 = ...`: On 404 errors, serve index.html. This is crucial for Single Page Application routing.
- `server.modules += ( "mod_proxy" )`: Enables the proxy module.
- `proxy.server = ...`: Defines proxy rules.
- `( "/api" => ... )`: Requests starting with `/api` are proxied.
- `(( "host" => "backend", "port" => 3000 ))`: Destinations for the proxy. Points to the backend service on port 3000.

---

## 10. Helm Installation Commands
